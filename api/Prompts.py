...
1. **Understand the Desired Properties:**
   - For each item in the output schema, identify the desired properties that the model's output should satisfy.

2. **Analyze Model Outputs:**
   - For each output item, compare the model's output against the desired properties.
   - Use chain-of-thought reasoning to think step by step about how well the output satisfies each desired property.

3. **Assign Scores:**
   - **Scoring Scale (0-100):**
     - **0-25:** The output does not satisfy the desired property at all.
     - **26-75:** The output partially satisfies the desired property.
     - **76-100:** The output fully satisfies the desired property. THIS SHOULD BE RARE.
   ...
   - Make sure the score is representative of how much a desired property is satisfied. If it is a negative property, like how wrong an answer it is, make sure that a higher score represents a more wrong answer.
...
*Evaluation:*
{
  "Answer": {
    ...
    "reasoning": "Mathematically correct (Score: 90):
...
def evaluation_prompt(seed=None):
    ...
    introductions = [
        "Given the task, input, and output schema outlining the metrics to grade for each item. Grade the output based on each metric and output the scores.",
        "Your task is to evaluate the given outputs based on the provided task, input, and output schema. For each metric, assign scores reflecting how well the outputs meet the desired properties.",
        "Evaluate the outputs by comparing them to the desired properties outlined in the output schema. For each metric, provide scores based on your assessment.",
    ]
    ...
    instructions_steps = [
        [
            "1. **Understand the Desired Properties:**\n   - For each item in the output schema, identify the desired properties that the model's output should satisfy.",
            "1. **Identify Desired Properties:**\n   - Review the output schema to find the desired properties for each item.",
            "1. **Review Output Schema:**\n   - Extract the desired properties for each item from the output schema.",
        ],
        [
            "2. **Analyze Model Outputs:**\n   - For each output item, compare the model's output against the desired properties.\n   - Use chain-of-thought reasoning to think step by step about how well the output satisfies each desired property.",
            "2. **Evaluate Outputs:**\n   - Assess each output item against the desired properties.\n   - Internally reason through how well each property is satisfied.",
            "2. **Assess Outputs:**\n   - Compare the outputs to the desired properties step by step using chain-of-thought reasoning.",
        ],
        [
            "3. **Assign Scores:**\n   - **Scoring Scale (0-100):**\n     - **0-25:** The output does not satisfy the desired property at all.\n     - **26-75:** The output partially satisfies the desired property.\n     - **76-100:** The output fully satisfies the desired property. THIS SHOULD BE RARE.\n   - Ensure that you use the full range of scores to reflect varying degrees of satisfaction.\n   - Aim for an average score of around 50 across all evaluations to maintain a balanced assessment.\n   - Make sure the score is representative of how much a desired property is satisfied. If it is a negative property, like how wrong an answer it is, make sure that a higher score represents a more wrong answer.",
            "3. **Scoring:**\n   - **Scale (0-100):**\n     - **0-25:** Desired property not satisfied at all.\n     - **26-75:** Desired property partially satisfied.\n     - **76-100:** Desired property fully satisfied (rare).\n   - Use the entire range to show varying degrees.\n   - Target an average score of about 50 to keep assessments balanced.\n   - Ensure higher scores represent a greater degree of satisfaction or deviation, depending on the context.",
            "3. **Provide Scores:**\n   - **Score Range (0-100):**\n     - **0-25:** Does not meet the desired property.\n     - **26-75:** Partially meets the desired property.\n     - **76-100:** Fully meets the desired property (should be uncommon).\n   - Use the full scoring spectrum for nuanced evaluations.\n   - Keep the average score around 50 for balance.\n   - Ensure scores accurately reflect satisfaction levels; for negative properties, higher scores indicate greater deviation.",
        ],
        [
            "4. **Maintain Original Outputs:**\n   - Keep the original model outputs exactly as they are provided.",
